...
export async function getCurrentAdminStatus() {
  const data = await must(supabase.auth.getUser(), "Kullanıcı bilgisi alınamadı");
...
export async function fetchBooks() {
  const rows = await must(supabase.from("books").select("*"), "Kitaplar alınamadı");
  return rows ?? [];
}
...
export async function fetchPolls() {
  const polls = await must(supabase.from("polls").select("*"), "Anketler alınamadı");
